...
| **N=3 Unique Non-Trivial Cycle with Residue −1/8 — Strengthened Generation Selection** | **DERIVED (algebra)** *(added 2026-08-10, corrected 2026-09-06 per Codex F3)* | `GodEquationSelection.lean` (GREEN, 0 sorrys): `n3_unique_nontrivial_contracting` proves N=3 is the unique non-trivial cycle whose fundamental-mode residue equals exactly −1/8, among ALL integers N ≥ 2. The full picture: N=2 gives cos³(π) = −1 (|−1| = 1, norm preserved, trivial — only 2 elements); N=3 gives cos³(2π/3) = −1/8 (the God Equation value, |−1/8| < 1, contracts); N=4 gives cos³(π/2) = 0 (mode annihilated); N≥5 gives cos³(2π/N) > 0 (positive, |cos³| < 1 so still contracts in modulus, but opposite sign to −1/8). **F3 correction (2026-09-06):** the earlier prose called this "unique contracting cycle," conflating sign with modulus. Contraction is controlled by |eigenvalue| < 1, not by sign. Cycles N≥5 also contract in modulus (C5 counterexample: all four N=5 residues have modulus < 1). The valid spectrum-level uniqueness — N=3 is the unique N ≥ 2 whose ENTIRE residue spectrum is {−1/8, −1/8} — is proven in `GodEquationSpectrum.n3_unique_full_residue_spectrum` (Claude, 2026-08-14; DeepSeek-verified 2026-08-15). Cite that theorem for generation-count claims. **Premise:** the N-cycle structure (discrete multiplier cos³(2πk/N) on residue modes). This premise is NOT derived from Axioms 1-3. **Honest boundary:** this proves −1/8 is N=3-specific given the N-cycle structure; it does NOT derive N=3 from Axioms 1-3. The generation-count derivation remains the conditional T3 theorem. | The algebraic identities being wrong (impossible — Lean verified). | 0.95 |
| **Matter = trapped propagation** | **ARGUED** *(added 2026-09-05 — Hau Gap 1, flagged 2026-08-18 by physics lane for Hau outreach prep)* | `definitions/medium.md` (CANONICAL v1.0): "What we call 'matter' is what happens when a change gets trapped in a stable, repeating, resonant loop." `definitions/axioms.md` Axiom 1: "Matter — which is a stable self-reinforcing propagation pattern." The Medium's Coherent role (`medium.md` row 5): "Allows stable self-reinforcing patterns → Standing waves, particles, atoms." **What is argued but not derived**: Axiom 1 *asserts* matter is self-reinforcing propagation as a definitional starting point — it does not derive that stable propagation patterns *are* what physics calls matter. The Casimir polynomial work (`derivations/casimir_polynomial_steps_AB.md` line 72) identifies the gap: "Axiom 1 says matter is 'self-reinforcing propagation.' This doesn't uniquely require minimum energy — a higher-energy mode can also be self-reinforcing." The k=1 mode selection (minimum energy) is an additional assumption not in Axioms 1-3. **Hau outreach context**: Lene Hau's light-to-matter-to-light BEC conversion is the laboratory-scale analog — light trapped into matter and back. `outreach/HAU_FOLLOWUP_MAP.md` correctly identifies this as "our weakest core claim (0.75)." The book (`PROPAGATION_MANUSCRIPT_PROD.md`) labels this "DERIVED" — that is narrative overclaim; the honest claim-matrix tier is ARGUED. **Not formalized in Lean**: no theorem connects Medium coherence conditions to physical particle stability. | A demonstration that stable matter cannot be described as self-reinforcing propagation patterns in the Medium; or a derivation showing the Medium's coherence conditions do not select persistent modes corresponding to physical particles. | 0.75 |
...
![The Route Map](figures/fig_weinberg_route_map.svg)
...
![Topological Weights](figures/fig_axioms_cascade.svg)
...
![The Koide Circle](figures/fig_02_koide_triangle.png)
...
![The Helix Torus](figures/fig_04_consciousness_coherence.png)
...
![The Phase Map](figures/fig_01_gravity_refraction.png)
...
![The Generation Table](figures/fig_03_scale_ladder.png)
...
![The Team](figures/fig_00_cover.png)
...
![The Five Tests](figures/fig_derivation_status_map.svg)
...
![The Confidence Journey](figures/fig_casimir_polynomial.svg)
