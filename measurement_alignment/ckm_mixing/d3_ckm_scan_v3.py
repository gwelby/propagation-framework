#!/usr/bin/env python3
"""
D3 v3: CKM Angle Scan — Branch-Continuation + Pre-Registered Sensitivity Study
...
4. FULL UNCERTAINTY PROPAGATION: Mass uncertainties, CKM parameter
   uncertainties, and root-selection ambiguity propagated via Monte Carlo.
5. PRE-REGISTRATION: Branch, statistic, and threshold declared before running.
...
# sin(θ₁₂) and sin(θ₁₃) uncertainties (symmetric, PDG 2024)
S12_SIG = 0.00029
S13_SIG = 0.000119
...
# Asymmetric: use average for symmetric propagation, report both
THETA_23_SIG_PLUS = S23_SIG_PLUS / C23
THETA_23_SIG_MINUS = S23_SIG_MINUS / C23
THETA_23_SIG = (THETA_23_SIG_PLUS + THETA_23_SIG_MINUS) / 2  # average for MC
...
# t: 172.5 ± 0.7 GeV (1σ) → 700 MeV (pole mass, cross-section extraction)
...
def monte_carlo_branch_tracking(
    theta_d_end: float, theta_u_end: float,
    n_draws: int = 5000, seed: int = 20260712,
    k: float = 1.0,
) -> Dict:
    """
    Monte Carlo over mass and CKM uncertainties.

    For each draw:
    1. Perturb masses within their 1-sigma uncertainties
    2. Perturb CKM parameters within their uncertainties
    3. Re-derive FX angles from perturbed CKM
    4. Track the branch from paper checkpoint to the perturbed endpoint
    5. Record the final θ₂₃ prediction

    This propagates mass, CKM, and root-selection ambiguities honestly.
    """
    rng = np.random.default_rng(seed)
    results = []

    # PDG 2024 FX angles (central)
    V_ckm_central = V_CKM_standard(S12, S23, S13, DELTA_CP)
    fx_central = extract_FX_angles(V_ckm_central)
    ...
    for draw_idx in range(n_draws):
        ...
        # Perturb CKM parameters
        s12_d = S12 + rng.normal(0, S12_SIG)
        s13_d = S13 + rng.normal(0, S13_SIG)
        delta_d = DELTA_CP + rng.normal(0, DELTA_CP_SIG)

        # Asymmetric θ₂₃: sample from a split-normal
        u = rng.uniform()
        if u < 0.5:
            s23_d = S23 + abs(rng.normal(0, S23_SIG_MINUS)) * (2 * u)
        else:
            s23_d = S23 + abs(rng.normal(0, S23_SIG_PLUS)) * (2 * (1 - u))
        ...
        # Derive FX angles from perturbed CKM
        try:
        ...
        # Track branch from paper checkpoint to perturbed endpoint
        result = track_branch_interpolation(
            ...
            td, tu,
            k=k,
            n_steps=20,  # fewer steps for MC speed
        )
    ...
    if len(results) < 100:
        return {
            ...
            "n_draws": n_draws,
            "error": "Too few successful draws for statistics",
        }
...
def main():
    L = []
    L.append("# D3 v3: CKM Angle Scan — Branch Continuation + Sensitivity Study")
    L.append("*Devin · 2026-07-12 · Zenczykowski 2013 (arXiv:1301.4143v2) · PDG 2024*")
    L.append("*Corrected per Codex audit CODEX_20260711_D3V2_CKM_PSEUDOMASS_AUDIT.md*")
    L.append("")
    ...
    # Corrections from v2
    L.append("## Corrections from D3 v2")
    L.append("")
    L.append("D3 v2 was CONDITIONAL PASS for source replay but REJECTED for current-data claims. Five fixes:")
    ...
    L.append("4. **Full uncertainty propagation:** Monte Carlo over mass + CKM parameter uncertainties + root-selection ambiguity")
    L.append("5. **Pre-registration:** Branch, statistic, and threshold declared above, before any computation")
    L.append("")
    ...
    rows.append(["sin(θ₁₃)", f"{S13:.6f}", f"±{S13_SIG:.6f}", "PDG 2024 Eq. 12.28"])
    ...
    if "error" in result_4b:
        ...
        L.append(f"**Result:** The paper branch ends at {result_4b['final_theta_23_deg']:.4f}° under "
                f"full PDG 2024 inputs (mixed-scale masses + PDG 2024 FX angles).")
        L.append("")
    ...
    # Monte Carlo uncertainty propagation
    # ============================================================
    L.append("## 5. Monte Carlo Uncertainty Propagation")
    L.append("")
    L.append("500 draws perturbing mass uncertainties (1σ Gaussian) and CKM parameter ")
    L.append("uncertainties (including asymmetric θ₂₃). For each draw, the branch is tracked ")
    L.append("from the paper checkpoint to the perturbed endpoint.")
    L.append("")

    print("Running Monte Carlo (500 draws, k=1)...")
    mc_result = monte_carlo_branch_tracking(
        theta_d_pdg, theta_u_pdg,
        n_draws=500, seed=20260712, k=1.0,
    )

    if "error" in mc_result:
        L.append(f"**Monte Carlo error:** {mc_result['error']}")
    else:
        L.append(f"**Successful draws:** {mc_result['n_successful']} / {mc_result['n_draws']}")
        L.append("")
        rows = [["Statistic", "Value (°)"]]
        rows.append(["Mean", f"{mc_result['mean']:.4f}"])
        rows.append(["Std", f"{mc_result['std']:.4f}"])
        rows.append(["Median", f"{mc_result['median']:.4f}"])
        rows.append(["5th percentile", f"{mc_result['p5']:.4f}"])
        rows.append(["95th percentile", f"{mc_result['p95']:.4f}"])
        rows.append(["Min", f"{mc_result['min']:.4f}"])
        rows.append(["Max", f"{mc_result['max']:.4f}"])
        L.append(fmt_table(rows))
        L.append("")
        ...
        L.append("**This is a sensitivity study, not a statistical test.** "
                "The Monte Carlo shows the spread of the branch-tracked prediction "
                "under mass and CKM parameter uncertainties. It does not account for "
                "the dominant uncontrolled systematic: masses from incompatible "
                "renormalization scales. No sigma-based claim is made.")
        L.append("")
    ...
    L.append("- Monte Carlo propagates mass + CKM + root-selection uncertainties")
    L.append("- Analysis pre-registered before computation")
    L.append("")
    ...
    if result_4b and "error" not in result_4b:
        ...
        L.append(f"   Observed θ₂₃ = {observed:.3f}°. The branch-tracked prediction is "
    ...
    if mc_result and "error" not in mc_result:
        L.append(f"3. **Uncertainty spread:** Monte Carlo (500 draws) gives "
                f"θ₂₃ = {mc_result['mean']:.2f}° ± {mc_result['std']:.2f}° "
                f"(90% CI: {mc_result['p5']:.2f}° to {mc_result['p95']:.2f}°). "
                f"This spread reflects mass and CKM parameter uncertainties but NOT "
                f"the scale-mixing systematic, which is uncontrolled.")
        L.append("")
    ...
    L.append("- Monte Carlo: 500 draws, independent Gaussian mass perturbations, "
             "asymmetric θ₂₃ perturbation, 20 continuation steps per draw")
    L.append("- Source script: `d3_ckm_scan_v3.py` in this directory")
    L.append("")

    # JSON output for audit
    output = {
        "schema": "devin-d3-v3-results",
        ...
        "pdg_2024_ckm": {
            ...
            "s12_sig": S12_SIG, "s13_sig": S13_SIG,
        },
        "fx_angles_pdg_2024": {
        ...
        "monte_carlo": mc_result if "error" not in mc_result else {"error": mc_result.get("error")},
    ...
    json_path = "/mnt/d/Fundamentals/measurement_alignment/ckm_mixing/d3_v3_results.json"
    with open(json_path, "w") as f:
        json.dump(output, f, indent=2, default=str)

    text = "\n".join(L)
    with open("D3v3_ckm_results.md", "w", encoding="utf-8") as f:
        f.write(text)
    print("Wrote D3v3_ckm_results.md")
    ...
    if mc_result and "error" not in mc_result:
        print(f"MC (500 draws): {mc_result['mean']:.2f}° ± {mc_result['std']:.2f}° "
              f"(90% CI: {mc_result['p5']:.2f}° to {mc_result['p95']:.2f}°)")
