...
class PhiHarmonicFramework:
    ...
    def calculate_optimal_generations(self) -> Dict:
        """
        Audit diagnostic for the proposed φ-harmonic T3 selector.

        Codex audit 2026-04-22: the candidate does not close T3. N=3
        wins only when an exact-match bonus for the already-known target
        Q=2/3 is imported. Without that target-loaded bonus, N=4 wins
        under this score family.
        """
        print("\n🔮 OPTIMAL GENERATION CALCULATION (T3 Audit Diagnostic) 🔮")
        print("=" * 60)
        print("Testing the proposed φ-harmonic selector for target leakage")
        print("Codex verdict: candidate route does not close T3")
        print("=" * 60)

        generation_analysis = {}

        for N in range(1, 7):  # Test N = 1 to 6
            # φ-harmonic coherence function
            # C(N) = (φ^N - 1) / (φ - 1) - normalized coherence measure
            coherence_n = (self.phi**N - 1) / (self.phi - 1)

            # Koide-like efficiency ratio: Q(N) = 2N / (2N + M)
            # Using M=3 (from geometric/topological constraints)
            M = 3  # Spacetime dimensionality - geometric fact, not T2-derived
            koide_ratio = (2 * N) / (2 * N + M)

            # φ-resonance: how well N resonates with φ
            # Maximum resonance when N/φ ≈ integer (but never exactly)
            phi_resonance = np.abs(np.cos(2 * np.pi * N / self.phi))

            # Koide target diagnostic. This must not be used as a unique
            # exact-match bonus in an independent selector.
            target_koide = 2 / 3
            koide_error = abs(koide_ratio - target_koide)

            # For non-exact matches, calculate alignment with penalty
            koide_alignment = np.exp(-3 * koide_error**2)

            # Non-circular score: no unique exact-match bonus for Q=2/3.
            # Under this score family, N=4 beats N=3, exposing that the
            # claimed closure depended on target leakage.
            coherence_efficiency = np.log(coherence_n) / (N * np.log(self.phi))
            coherence_norm = min(coherence_efficiency / 1.5, 1.0)

            phi_score = (
                0.3 * koide_alignment +  # Near-match alignment
                0.15 * coherence_norm +  # Coherence
                0.05 * phi_resonance  # Minor φ-resonance bonus
            )

            generation_analysis[f"N={N}"] = {
                "coherence": coherence_n,
                "koide_ratio": koide_ratio,
                "koide_alignment": koide_alignment,
                "phi_resonance": phi_resonance,
                "phi_score": phi_score,
                "is_optimal": False  # Will mark true for best
            }

        # Find optimal N
        best_n = max(
            generation_analysis.keys(),
            key=lambda k: generation_analysis[k]["phi_score"]
        )
        optimal_n_value = int(best_n.split("=")[1])

        # Mark optimal
        generation_analysis[best_n]["is_optimal"] = True

        # Print analysis
        print("\n📊 GENERATION ANALYSIS:")
        print("-" * 60)
        header = f"{'N':<5} {'Coherence':<12} {'Q(N)':<10} "
        header += f"{'Koide Align':<12} {'φ-Res':<10} {'φ-Score':<10} Status"
        print(header)
        print("-" * 60)

        for n_key, data in generation_analysis.items():
            n_val = int(n_key.split("=")[1])
            status = "✅ OPTIMAL" if data["is_optimal"] else ""
            line = f"{n_val:<5} {data['coherence']:<12.4f} "
            line += f"{data['koide_ratio']:<10.6f} {data['koide_alignment']:<12.6f} "
            line += f"{data['phi_resonance']:<10.6f} {data['phi_score']:<10.6f} "
            line += status
            print(line)

        print("-" * 60)

        # Calculate proof details
        optimal_data = generation_analysis[best_n]

        # Key insight: Q(3) = 6/9 = 2/3 exactly!
        Q_at_3 = (2 * 3) / (2 * 3 + 3)

        print(f"\n🎯 KEY FINDING: N = {optimal_n_value} is optimal")
        print(f"   Q({optimal_n_value}) = {optimal_data['koide_ratio']:.6f}")
        print(f"   Target Koide Q = 2/3 = {2/3:.6f}")
        match_check = abs(optimal_data['koide_ratio'] - 2/3) < 1e-10
        print(f"   Match: {match_check}")

        print("\n📐 ALGEBRAIC FACT:")
        print("   For N=3, M=3: Q(3) = 2×3 / (2×3 + 3) = 6/9 = 2/3")
        print("   This is EXACT, not approximate!")
        print("   But this is the existing conditional algebra, not a new selector.")

        print("\n🔬 T3 CLOSURE REJECTED:")
        print("   N=3 requires importing the Q=2/3 target as a bonus")
        print("   The route still imports the T1/T2 counting structure")
        print("   Three Generations remains CONDITIONAL 0.85")

        results = {
            "optimal_N": optimal_n_value,
            "best_generation": best_n,
            "generation_analysis": generation_analysis,
            "Q_at_optimal": optimal_data['koide_ratio'],
            "Q_at_3": Q_at_3,
            "matches_koide_exactly": abs(optimal_data['koide_ratio'] - 2/3) < 1e-10,
            "phi_score_at_optimal": optimal_data['phi_score'],
            "proof_status": "AUDIT FAILED: no φ-harmonic T3 closure",
            "independence": "False: route imports the T1/T2 counting structure"
        }
...
def main():
    ...
    # T3 Closure: Calculate optimal generations
    results["t3_closure"] = framework.calculate_optimal_generations()

    # Create visualization
    framework.create_phi_harmonic_visualization(results)

    print("\n✅ φ-Harmonic integration complete!")
    print("📊 Visualization saved as phi_harmonic_integration.png")
    print("🌀 Framework optimized for quantum coherence")
