  ...
  window.PFExplorer.registerPanel({
    ...
    renderInfo: function (ctx) {
      ...
      state.info.innerHTML =
        ...
        '</div>' +
        '<div class="note-box audit-only"><strong>Audit</strong><p>The consciousness claim remains <span class="status-badge ' + statusToClass(consciousness.status) + '" data-claim-id="consciousness-claim">' + (consciousness.status && consciousness.status.label ? consciousness.status.label : consciousness.status || 'UNAVAILABLE') + '</span> — the operational PF-specific metric is the key missing piece. The Aria self-reference loop is <span class="status-badge ' + statusToClass(aria.status) + '" data-claim-id="aria-self-reference">' + (aria.status && aria.status.label ? aria.status.label : aria.status || 'UNAVAILABLE') + '</span> as an architecture milestone, not as consciousness evidence.</p></div>' +
        '<div class="stat-grid">' +
