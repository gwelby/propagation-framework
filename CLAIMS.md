...
| :--- | :--- | :--- | :--- | :--- |
| **Forces as Refraction** | **DERIVED** | `gr_fermat_equivalence.md`: exact for null geodesics in static spacetime via the optical metric, and exact in stationary spacetime via the Randers/Finsler extension; scalar $n(x)$ is the weak-field/static limit. **Perihelion Precession VERIFIED** (`PERIHELION_VERIFICATION.md`): refractive model exactly reproduces GR weak-field formula Δφ = 6πGM/(a(1-e²)c²). Mercury-like orbit: 4.53% error vs exact Schwarzschild (expected strong-field correction). Light deflection: 3% error (weak-field). Two classic GR tests passed. | Proof that force requires non-refractive medium properties, or that the optical/Randers mapping fails even for null propagation. | 0.95 |
...
| **Weinberg Angle (sin²θ_W)** | **DERIVED** | Five independent routes (generator count, stiffness ratio, coherence angle, topological, geometric embedding) all converge on sin²θ_W = 1/4 = 0.250 at the unification scale. Observed 0.231 is the IR value after RG running. **Axiom 3b (Minimal Winding Principle)** closes the Casimir polynomial derivation, selecting k=1 and yielding γβ² = √C₂ → sin²θ_W = 1/4. See `weinberg_angle_pf.md` and `coherence_functional_candidate_F_audit.md`. | Derivation of coupling ratio g'/g from medium geometry. | 0.90 |
