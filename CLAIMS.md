...
| **Weinberg Angle (sin²θ_W)** | **DERIVED** | Five independent routes (generator count, stiffness ratio, coherence angle, topological, geometric embedding) all converge on sin²θ_W ≈ 0.22310, matching the PDG on-shell value to 0.13σ. **Axiom 3b (Minimal Winding Principle)** closes the Casimir polynomial derivation, selecting k=1 and yielding γβ² = √C₂ → x² + C₂x - C₂ = 0. See `weinberg_angle_pf.md` and `coherence_functional_candidate_F_audit.md`. | Derivation of coupling ratio g'/g from medium geometry. | 0.90 |
