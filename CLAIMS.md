...
| **Weinberg Angle (sin²θ_W)** | **ARGUED** | PF gauge structure SO(3)×U(1) gives natural prediction sin²θ_W = 1/4 = 0.250 from generator counting (three independent routes: count ratio, stiffness ratio, coherence angle). **Observed value 0.231 is 8% below this prediction.** The Casimir polynomial derivation (k=1 selection via Axiom 3b) is DERIVED, but the numerical Weinberg angle value requires either RG running (not in PF) or relative normalization of SO(3)/U(1) generators (underived). See `weinberg_angle_pf.md` for honest failure analysis. | Derivation of coupling ratio g'/g from medium geometry, or RG running mechanism. | 0.60 |
