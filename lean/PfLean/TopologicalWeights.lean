...
/-
  ...
  Date: 2026-06-04 (fixed 2026-06-14)
  ...
  - The kernel of the double cover SU(2) → SO(3) is {±1} (proven in
    SO3DoubleCover.lean via quatToSO3_ker)
  - Elements of the kernel have closure orders 1 and 2 exactly
    (at_most_two_closure_orders — proven)
  - The topological availability theorem (kernel only): if g acts trivially
    on SO(3), then g has order 1 or 2 (topological_availability — proven)

  WHAT IS CONDITIONAL (honest boundary):
  - The full textbook covering-space route (π₁(SO(3)) ≅ ℤ₂ via path lifting
    and deck transformations) is not formalized in this file.
    NOTE: This full route is overkill for the PF claim. The kernel-only
    theorem (proven here) is sufficient for the local obstruction.
  ...
  STATUS: The algebraic kernel obstruction is proven (zero sorrys).
  The full topological wrapping and physical realization remain
...
-- ---------------------------------------------------------------------------
-- 3. Honest Boundary — What We Can Prove (Kernel Only)
-- ---------------------------------------------------------------------------
...
-- ---------------------------------------------------------------------------
-- 4. The Classification Theorem (Kernel Only)
-- ---------------------------------------------------------------------------

/-- **Theorem (Kernel Classification — PROVEN):** The kernel of the
    quaternion-to-SO(3) covering map has exactly two elements, with
    closure orders 1 and 2.

    This is the honest algebraic foundation: quatToSO3 g = 1 ↔ g = 1 ∨ g = -1
    (proven in SO3DoubleCover.lean), and these are the ONLY elements that
    act trivially on SO(3).

    The full textbook covering-space route (π₁(SO(3)) ≅ ℤ₂ via path lifting
    and deck transformations) is not formalized in this file. The kernel-only
    theorem (proven here) is sufficient for the PF (2,1) local obstruction. -/
theorem kernel_closure_orders :
  ∀ g : UnitQuaternion, quatToSO3 g = 1 → closureOrder g = 1 ∨ closureOrder g = 2 := by
  intro g h_ker
  -- quatToSO3 g = 1 ↔ g = 1 ∨ g = -1 (proven in SO3DoubleCover.lean)
  have h_g : g = 1 ∨ g = -1 := by
    exact (quatToSO3_ker g).mp h_ker
  -- The kernel has only two elements, with orders 1 and 2
  exact at_most_two_closure_orders g h_g

/-- **The Topological Availability Theorem (PROVEN):**
    The kernel of the covering map SU(2) → SO(3) is exactly {±1}.
    Therefore, the only closure-order classes that act trivially on SO(3) are {1, 2}.

    This does NOT require path lifting or covering-space theory.
    The proof uses only the algebraic kernel obstruction:
    quatToSO3 g = 1 ↔ g = 1 ∨ g = -1 (proven in SO3DoubleCover.lean).

    Status (2026-06-14): Previously blocked by a false theorem statement that
    claimed ALL UnitQuaternion elements have order 1 or 2. The correct claim
    is about the kernel only. Path B analysis by Devin ∇λΣ∞ found the fix. -/
theorem topological_availability :
  ∀ g : UnitQuaternion, quatToSO3 g = 1 → closureOrder g = 1 ∨ closureOrder g = 2 :=
  kernel_closure_orders

-- ---------------------------------------------------------------------------
-- 5. The (2,1) Weights Claim — Explicit Statement
-- ---------------------------------------------------------------------------
