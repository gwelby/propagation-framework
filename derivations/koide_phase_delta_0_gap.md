...
The gap of 7.4×10⁻⁶ is 0.89× the m_τ measurement uncertainty — i.e. δ sits **0.89σ** from 2/9, consistent, but near the 1σ edge. **Improved τ-mass precision is what threatens this row, not what confirms it:** **conditional on the current central value holding**, σ(m_τ)=0.03 MeV would put δ at 3.55σ and Q at 3.63σ, past both rows' own >3σ criterion. This is a conditional consequence, not a forecast — a future central value is unknown, and Q and δ share the τ input so they are not independent tests. See the pre-registered τ-mass trigger in `CLAIMS.md`. **Note (2026-09-04)**: the previous "0.029σ" was based on a stale uncertainty value 30.9× too large. The correct value is 0.89σ — still consistent, but much closer to the 1σ edge than previously stated. See Claude's Monte-Carlo verification (N=200k) and the τ-mass pre-registration trigger in CLAIMS.md.
